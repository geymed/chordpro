...
const STORAGE_KEY = 'chordvault-sheets';

export function getSheets(): ChordSheet[] {
  if (typeof window === 'undefined') return [];
  const stored = localStorage.getItem(STORAGE_KEY);
  if (!stored) return getDefaultSheets();
  try {
...
export function saveSheets(sheets: ChordSheet[]): void {
  if (typeof window === 'undefined') return;
...
function getDefaultSheets(): ChordSheet[] {
  return [
    {
      id: '1',
